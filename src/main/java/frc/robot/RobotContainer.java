...
public class RobotContainer {
  ...
  private void configureButtonBindings() {
    ...
    nt.addEntryListener("capture", (table, key, entry, value, flags) -> {
      if((boolean) value.getValue()){
        CommandScheduler.getInstance().schedule(new CaptureBall(m_drivetrain, m_gyro, false, ballCount));
        ballCount++;
      }
    }, EntryListenerFlags.kNew | EntryListenerFlags.kUpdate);
  ...
  public Command getLineFollowerCommand(){
    return new ArcadeDrive(m_drivetrain, () -> (-0.5), () -> (0.66*xCenterSteer));
  }
